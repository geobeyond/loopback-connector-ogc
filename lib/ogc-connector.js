...
/**
 * Find or derive the method name from service/version/request
 * @param {String} serviceName The getCapabilities service name
 * @param {String} versionName The getCapabilities version name
 * @param {String} requestName The getCapabilities request name
 * @param {Object} dao The data access objecr
 * @returns {String} The method name
 * @private
 */
OGCConnector.prototype._methodName = function (serviceName, versionName, requestName, dao) {
  var methods = this.settings.requests || {};
  for (var m in methods) {
    var method = methods[m];
    if (method.service === serviceName && method.version === versionName &&
      (method.request === requestName ||
        method.request === undefined && m === requestName)) {
      // Return the matched method name
      return m;
    }
  }

  if (dao && (requestName in dao)) {
    // The request name exists, return full name
    return serviceName + '_' + versionName + '_' + requestName;
  } else {
    return requestName;
  }
};
...
function findMethod(client, name) {
  var requestTypes = client.getCapabilities.definitions.requestTypes;
  for (var p in requestTypes) {
    var pt = requestTypes[p];
...
OGCConnector.prototype.xmlToJSON = function (method, xml) {
  ...
  if (typeof method === 'string') {
    var m = findMethod(this.client, method);
    if(!m) {
      throw new Error('Method not found in getCapabilities request types: ' + m);
    } else {
      method = m;
    }
  }
