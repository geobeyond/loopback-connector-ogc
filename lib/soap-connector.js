...
OGCConnector.prototype.connect = function (cb) {
  ...
  ows.createClient(self.wsdl, self.settings, function (err, client) {
    if (!err) {
      if (debug.enabled) {
      ...
      if (Array.isArray(self.settings.owsHeaders)) {
        self.settings.owsHeaders.forEach(function (header) {
          ...
          if (typeof header === 'object') {
            client.addOwsHeader(client.wsdl.objectToXML(header.element, header.name,
    ...
    // If the getCapabilities is cached, the callback from soap.createClient is sync (BUG!)
    if (cb) {
      process.nextTick(function () {
        cb(err, client);
      });
